...
} from "../utils/tailoredResumeValidator";
...
function Stage2Tailor({ primaryResume, onBack, onNavigate }) {
  ...
  const { bullets: primeBullets, loading: primeBulletsLoading } =
  ...
  const [downloadProgress, setDownloadProgress] = useState(""); // Progress message during download
  ...
  useEffect(() => {
    if (primeBullets && primeBullets.length > 0 && primeResume) {
      ...
      const primeResumeJson = primeBulletsToResumeJson(
        primeBullets,
      ...
      if (primaryResume) {
        ...
        primeResumeJson.contact = primeResumeJson.contact?.email
          ? primeResumeJson.contact
          : primaryResume.contact;
        primeResumeJson.education =
          primeResumeJson.education?.length > 0
            ? primeResumeJson.education
            : primaryResume.education;
        primeResumeJson.skills =
          primeResumeJson.skills?.length > 0
            ? primeResumeJson.skills
  ...
  const handleAnalyzeWithChecklist = async (e) => {
    ...
    try {
      ...
      console.log("Using resume for tailoring:", {
        ...
        bulletCount: resumeToUse?.experience?.reduce(
      ...
      const selectionResult = scoreAndSelectBullets(
        resumeToUse,
        extractedChecklist,
      );
      ...
      const { tailored, bullets } = await handleTailorSelectedBullets(
        extractedChecklist,
        selectionResult,
        resumeToUse,
      );
  ...
  const handleTailorSelectedBullets = async (
    checklistData,
    selectionData,
    resumeToUse,
  ) => {
    try {
      ...
      const bulletsWithContext = selectionData.selected_bullets.map(
        (bullet) => {
...
Rewrite these bullets to emphasize the must-haves and primary keywords while following the rules above. Return JSON only.`;
      ...
      const tailored = applyRewrittenBullets(
        resumeToUse,
  ...
  const handleValidation = async (tailored, checklistData, bullets) => {
    try {
      ...
      const validation = await validateTailoredResume(
        tailored,
        checklistData,
        bullets,
      );
  ...
  const applyRewrittenBullets = (resume, rewrittenBullets) => {
    ...
    rewrittenBullets.forEach(({ bullet_id, new_text }) => {
      ...
      if (match) {
        ...
        if (
          tailored.experience[expIndex] &&
          tailored.experience[expIndex].bullets[bulletIndex]
        ) {
    ...
    try {
      ...
      try {
        ...
        if (docError.code === "RESUME_OVERFLOW") {
          setDownloadProgress("Content overflow - auto-trimming...");
          console.log(
            `📏 Overflow detected - auto-trimming tailored resume...`,
          );

          // Calculate trim amount
          const linesToTrim = Math.ceil(docError.overflowPercent / 2);

          const trimPrompt = TRIM_PROMPT.replace(
            /{LINES_OVER}/g,
            linesToTrim.toString(),
          )
            .replace(
              "{BULLETS_TO_REMOVE}",
              Math.ceil(linesToTrim / 1.5).toString(),
            )
            .replace(
              "{BULLETS_TO_SHORTEN}",
              Math.ceil(linesToTrim * 1.5).toString(),
...
Please trim this resume to fit on 1 page. Return ONLY the JSON object with the trimmed resume data.`;
          ...
          setDownloadProgress("AI trimming content to fit 1 page...");
          ...
          if (trimMatch) {
            ...
            // Retry document generation with trimmed content
            setDownloadProgress(
              "Regenerating document with trimmed content...",
            );
            await generateDOCX(sortedTrimmed, null, companyName, null, {
              validatePageCount: true,
              maxValidationAttempts: 5,
              onProgress,
            });
            ...
            throw new Error(
              "Auto-trim failed - could not parse trimmed resume",
            );
          }
        } else {
          throw docError;
  ...
  return (
    <div className="container">
      <nav className="nav-bar">
        ...
        <div className="logo" style={{ fontSize: "24px", margin: 0 }}>
          May
        </div>
      </nav>
          ...
          >
            {primeBullets?.length > 0 ? (
              <>
                ...
                Using Prime Resume • {primeBullets.length} bullets from{" "}
                {scoringResumeData.experience?.length || 0} experiences
              </>
            ) : (
              <>
                Using resume for: {scoringResumeData.name || "Unknown"}
                {scoringResumeData.experience &&
                  ` • ${scoringResumeData.experience.length} experience entries`}
              </>
            )}
          </p>
        )}
        ...
        <>
          {!checklist && !tailoredResume && !isLoading && (
            <div className="card-premium">
              ...
              >
                Paste the full job posting below. May will extract requirements,
                score your bullets, and tailor your resume automatically.
              </p>
          ...
          {isLoading && (
            ...
            >
              ...
              >
                {loadingStage === "checklist" &&
                  "Extracting job requirements..."}
                {loadingStage === "scoring" &&
                  "Scoring and selecting bullets..."}
                {loadingStage === "tailoring" &&
                  "Tailoring selected bullets..."}
                {loadingStage === "validating" &&
                  "Validating tailored content..."}
              </p>
              <div
                className="loading"
            ...
            <div className="stagger-1">
              ...
              >
                ...
                >
                  ...
                  {checklist.job_metadata.company_name} -{" "}
                  {checklist.job_metadata.job_title}
                </p>
                <button
                ...
                {showChecklist && (
                  ...
                  >
                    <div style={{ marginBottom: "var(--space-sm)" }}>
                      ...
                      {checklist.must_haves.hard_skills.join(", ")}
                    </div>
                    <div style={{ marginBottom: "var(--space-sm)" }}>
                      <strong>Must-Have Tools:</strong>{" "}
                      {checklist.must_haves.tools.join(", ")}
                    </div>
                    <div>
                      <strong>Primary Keywords:</strong>{" "}
                      {checklist.keyword_pack.primary.join(", ")}
                    </div>
                  </div>
                )}
              </div>
              ...
              >
                <div className="card-title" style={{ color: "#065f46" }}>
                  <WritingIcon />
                  {selection.selected_bullets.length} Bullets Selected for
                  Tailoring
                </div>
                <p
                  style={{
                ...
                >
                  Coverage:{" "}
                  {selection.coverage_report.covered_must_haves.length} of{" "}
                  {selection.coverage_report.covered_must_haves.length +
                    selection.coverage_report.missing_must_haves.length}{" "}
                  must-haves covered
                </p>
                ...
                {selection.coverage_report.missing_must_haves.length > 0 && (
                  ...
                  >
                    <strong>Missing from resume:</strong>{" "}
                    {selection.coverage_report.missing_must_haves.join(", ")}
                  </div>
                )}
                ...
                {showSelection && (
                  <div style={{ marginTop: "var(--space-md)" }}>
                    {selection.selected_bullets
                      ...
                      .map((bullet, idx) => (
                        ...
                        >
                          ...
                          >
                            Score: {bullet.score} -{" "}
                            {bullet.matched_terms.slice(0, 3).join(", ")}
                          </div>
                    ...
                    {selection.selected_bullets.length > 5 && (
                      ...
                      >
                        ... and {selection.selected_bullets.length - 5} more
                        bullets
                      </div>
                    )}
                  </div>
          ...
          {tailoredResume && (
            <div className="stagger-1">
              ...
              >
                ...
                >
                  Resume Tailored for{" "}
                  {checklist?.job_metadata?.company_name || "This Role"}
                </h2>
                <p
                  style={{
                    fontSize: "16px",
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    >
                      {selection?.coverage_report?.covered_must_haves?.length ||
                        0}
                      /
                      {(selection?.coverage_report?.covered_must_haves
                        ?.length || 0) +
                        (selection?.coverage_report?.missing_must_haves
                          ?.length || 0)}
                    </div>
                    <div style={{ fontSize: "13px", color: "#6b7280" }}>
                      Must-Haves Covered
                ...
                {selection?.coverage_report?.missing_must_haves?.length > 0 && (
                  ...
                  >
                    <strong>Not in your experience:</strong>{" "}
                    {selection.coverage_report.missing_must_haves.join(", ")}
                  </div>
                )}
                ...
                >
                  ...
                  >
                    ...
                    >
                      <DownloadIcon />
                      {downloadProgress ? "Processing..." : "Download DOCX"}
                    </button>
                    <button
              ...
              >
                ...
                >
                  <div className="card-title" style={{ margin: 0 }}>
                    <WritingIcon />
                    What Changed ({selection?.selected_bullets?.length ||
                      0}{" "}
                    bullets)
                  </div>
                  <span
                ...
                {showComparison && (
                  <div style={{ marginTop: "var(--space-lg)" }}>
                    {selection?.selected_bullets?.map((selectedBullet, idx) => {
                      ...
                      const rewritten = rewrittenBullets?.find(
                        (b) => b.bullet_id === selectedBullet.bullet_id,
                      );
                      if (!rewritten) return null;

                      // Get company/role context
                      const match =
                        selectedBullet.bullet_id.match(/exp(\d+)_bullet(\d+)/);
                      let company = "";
                      let role = "";
                      if (match) {
                        const expIndex = parseInt(match[1], 10);
                        const exp = (scoringResumeData || primaryResume)
                          ?.experience?.[expIndex];
                        company = exp?.company || "";
                        role = exp?.title || "";
                      }
                      ...
                      return (
                        <div
                          key={selectedBullet.bullet_id}
                          style={{
                            marginBottom: "var(--space-lg)",
                            paddingBottom: "var(--space-lg)",
                            borderBottom:
                              idx < selection.selected_bullets.length - 1
                        ...
                        >
                          ...
                          <ul style={{ paddingLeft: "20px", margin: 0 }}>
                            {exp.bullets.map((bullet, bulletIdx) => {
                              ...
                              const wasChanged =
                                selection?.selected_bullets.some(
              ...
              >
                ...
                <button onClick={handleRetailor} className="btn btn-secondary">
                  ← Back to Analysis
                </button>
              </div>
            </div>
